...
O principal objetivo deste projeto é desenvolver, na plataforma \textit{Drupal}, um sistema \textit{web} de gerenciamento de biblioteca que atenda às necessidades do Curso Exato. Tal sistema deve seguir os padrões de código da comunidade \textit{Drupal}\footnote{um CMS e \textit{framework} (coleção de \textit{software} que fornece uma estrutura básica para outro sistema) de código aberto.}, para que possa ser estendido e utilizado por terceiros. Esta plataforma foi escolhida por ser de familiaridade dos desenvolvedores e dos membros do Curso Exato, e também por ser personalizável, extensível, de fácil manutenção, e de baixo custo aos futuros administradores da biblioteca.
...
06/11 a 18/12 & Melhorias finais \\
...
\item Lista de identificadores dos exemplos desta publicação na biblioteca
\end{itemize}
...
Ainda, foi habilitada a funcionalidade do \textit{Drupal} de fazer com que o cadastro passe por aprovação dos administradores para que o usuário anônimo ganhe acesso ao sistema. Estes são notificados por e-mail quando um novo cadastro é criado. A Figura \ref{cadastro} mostra como esta seção é exibida ao usuário anônimo.
...
Durante o desenvolvimento das telas anteriores foram encontrados problemas no fluxo de empréstimo quando certas ações eram realizadas por usuários autenticados. Especificamente, notou-se que usuários com empréstimos correntes não possuíam permissão para renová-los em qualquer situação - embora o link referente a esta operação estivesse disponível na página do usuário -, além de qualquer erro em operações ativadas a partir da página de usuário causar o redirecionamento para uma página indisponível a usuários não administradores.

Após curta investigação, foram encontradas e corrigidas as falhas no código (\textit{bugs}) que provocaram este comportamento inesperado. Julgou-se a não consideração de alguns cenários deste fluxo de funcionamento do ponto de vista dos usuários comuns como a causa raiz do problema.

\subsubsection{Preparação para implantação no servidor disponível para o Curso Exato}

Discutiu-se via e-mail sobre o local ideal para a implantação do sistema, dado algumas exigências e implicações por parte do Curso Exato. Após estudar as possibilidades e avaliar os pontos favoráveis e os contrários a cada opção, decidiu-se estabelecê-lo no servidor de projetos discentes do Instituto de Computação (IC) da Unicamp, de forma a lançá-lo o mais breve possível.
